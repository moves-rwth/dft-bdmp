#include <iostream>
#include "FigaroModelBDMP_02_3trains_standby_redundancy_No_trim_repair.h"
...
void storm::figaro::FigaroProgram_BDMP_02_3trains_standby_redundancy_No_trim_repair::init()
{
	...
	FIRE_xx11_OF_A_op = false;
	FIRE_xx23_OF_A_start_INS_2 = false;
	FIRE_xx23_OF_A_start_INS_3 = false;
	FIRE_xx24_OF_A_start = false;
	FIRE_xx10_OF_B_op = false;
	FIRE_xx11_OF_B_op = false;
	FIRE_xx10_OF_C_fail = false;
	FIRE_xx11_OF_C_fail = false;
	FIRE_xx23_OF_C_start_INS_9 = false;
	FIRE_xx23_OF_C_start_INS_10 = false;
	FIRE_xx24_OF_C_start = false;
	FIRE_xx10_OF_P_op = false;
	FIRE_xx11_OF_P_op = false;

}

void storm::figaro::FigaroProgram_BDMP_02_3trains_standby_redundancy_No_trim_repair::saveCurrentState()
{
...
int storm::figaro::FigaroProgram_BDMP_02_3trains_standby_redundancy_No_trim_repair::compareStates()
{
...
void storm::figaro::FigaroProgram_BDMP_02_3trains_standby_redundancy_No_trim_repair::printState()
{
...
bool storm::figaro::FigaroProgram_BDMP_02_3trains_standby_redundancy_No_trim_repair::figaromodelhasinstransitions()
{
	return true;
}
void storm::figaro::FigaroProgram_BDMP_02_3trains_standby_redundancy_No_trim_repair::doReinitialisations()
{
...
void storm::figaro::FigaroProgram_BDMP_02_3trains_standby_redundancy_No_trim_repair::fireOccurrence(int numFire)
{
	...
	if (numFire == 3)
	{
		FIRE_xx23_OF_A_start_INS_3 = true;
	}

	if (numFire == 4)
	{
		FIRE_xx24_OF_A_start = true;
	}
	...
	if (numFire == 8)
	{
		FIRE_xx11_OF_C_fail = true;
	}

	if (numFire == 9)
	{
		FIRE_xx23_OF_C_start_INS_9 = true;
	}

	if (numFire == 10)
	{
		FIRE_xx23_OF_C_start_INS_10 = true;
	}

	if (numFire == 11)
	{
		FIRE_xx24_OF_C_start = true;
	}
	...
	if (numFire == 13)
	{
		FIRE_xx11_OF_P_op = true;
	}

/* ---------- DECLARATION OF OCCURRENCE RULES------------ */

	// Occurrence xx10_OF_A_op
	if ((boolState[failF_OF_A_op] == false) && boolState[required_OF_A_op]) 
	{
	...
	// Occurrence xx11_OF_A_op
	if (boolState[failF_OF_A_op] == true) 
	{
		 
		if (FIRE_xx11_OF_A_op)
		{
			boolState[failF_OF_A_op]  =  false;
			FIRE_xx11_OF_A_op = false;
		}
	...
	// Occurrence xx24_OF_A_start
	if (boolState[failI_OF_A_start] == true) 
	{
		 
		if (FIRE_xx24_OF_A_start)
		{
			boolState[failI_OF_A_start]  =  false;
			FIRE_xx24_OF_A_start = false;
		}
	...
	if ((boolState[failF_OF_B_op] == false) && boolState[required_OF_B_op]) 
	{
	...
	// Occurrence xx11_OF_B_op
	if (boolState[failF_OF_B_op] == true) 
	{
		 
		if (FIRE_xx11_OF_B_op)
		{
			boolState[failF_OF_B_op]  =  false;
			FIRE_xx11_OF_B_op = false;
		}
	...
	if ((boolState[failF_OF_C_fail] == false) && boolState[required_OF_C_fail]) 
	{
	...
	// Occurrence xx11_OF_C_fail
	if (boolState[failF_OF_C_fail] == true) 
	{
		 
		if (FIRE_xx11_OF_C_fail)
		{
			boolState[failF_OF_C_fail]  =  false;
			FIRE_xx11_OF_C_fail = false;
		}
	...
	if ((boolState[failI_OF_C_start] == false) && boolState[to_be_fired_OF_C_start]) 
	{
		...
		if (FIRE_xx23_OF_C_start_INS_10) 
		{
			boolState[already_standby_OF_C_start]  =  false;
			boolState[already_required_OF_C_start]  =  false;
			FIRE_xx23_OF_C_start_INS_10 = false;
	...
	// Occurrence xx24_OF_C_start
	if (boolState[failI_OF_C_start] == true) 
	{
		 
		if (FIRE_xx24_OF_C_start)
		{
			boolState[failI_OF_C_start]  =  false;
			FIRE_xx24_OF_C_start = false;
		}
	...
	if ((boolState[failF_OF_P_op] == false) && boolState[required_OF_P_op]) 
	{
	...
	// Occurrence xx11_OF_P_op
	if (boolState[failF_OF_P_op] == true) 
	{
		 
		if (FIRE_xx11_OF_P_op)
		{
			boolState[failF_OF_P_op]  =  false;
			FIRE_xx11_OF_P_op = false;
		}
...
std::vector<std::tuple<int, double, std::string, int>> storm::figaro::FigaroProgram_BDMP_02_3trains_standby_redundancy_No_trim_repair::showFireableOccurrences()
{
	...
	if ((boolState[failI_OF_A_start] == false) && boolState[to_be_fired_OF_A_start])
	{
		//cout << "2 :  INS_SUB_COUNT (1) |FAULT | failI  LABEL \"instantaneous failure A_start\" | DIST INS (0.0001) | INDUCING boolState[failI_OF_A_start]  =  TRUE,already_standby_OF_A_start  =  FALSE,already_required_OF_A_start  =  FALSE" << endl;
		list.push_back(make_tuple(2, 0.0001, "INS", 1));
	}
	if ((boolState[failI_OF_A_start] == false) && boolState[to_be_fired_OF_A_start])
	{
		//cout << "3 :  INS_SUB_COUNT (1) |TRANSITION | good | DIST INS (0.9999) | INDUCING boolState[already_standby_OF_A_start]  =  FALSE,already_required_OF_A_start  =  FALSE" << endl;
		list.push_back(make_tuple(3, 0.9999, "INS", 1));
	}
	if ((boolState[failI_OF_C_start] == false) && boolState[to_be_fired_OF_C_start])
	{
		//cout << "9 :  INS_SUB_COUNT (2) |FAULT | failI  LABEL \"instantaneous failure C_start\" | DIST INS (0.0001) | INDUCING boolState[failI_OF_C_start]  =  TRUE,already_standby_OF_C_start  =  FALSE,already_required_OF_C_start  =  FALSE" << endl;
		list.push_back(make_tuple(9, 0.0001, "INS", 2));
	}
	if ((boolState[failI_OF_C_start] == false) && boolState[to_be_fired_OF_C_start])
	{
		//cout << "10 :  INS_SUB_COUNT (2) |TRANSITION | good | DIST INS (0.9999) | INDUCING boolState[already_standby_OF_C_start]  =  FALSE,already_required_OF_C_start  =  FALSE" << endl;
		list.push_back(make_tuple(10, 0.9999, "INS", 2));
	}
        if (list.size() > 0)
        {
	...
	if (boolState[failF_OF_A_op] == true)
	{
		//cout << "1 : xx11_OF_A_op : REPAIR rep  DIST EXP (0.1)  INDUCING boolState[failF_OF_A_op]  =  FALSE" << endl;
		list.push_back(make_tuple(1, 0.1, "EXP", 0));
	}
	if (boolState[failI_OF_A_start] == true)
	{
		//cout << "4 : xx24_OF_A_start : REPAIR rep  DIST EXP (0.1)  INDUCING boolState[failI_OF_A_start]  =  FALSE" << endl;
		list.push_back(make_tuple(4, 0.1, "EXP", 0));
	}
	if ((boolState[failF_OF_B_op] == false) && boolState[required_OF_B_op])
	{
		//cout << "5 : xx10_OF_B_op : FAULT failF  LABEL \"failure in operation B_op\"  DIST EXP (0.0001)  INDUCING boolState[failF_OF_B_op]  =  TRUE" << endl;
		list.push_back(make_tuple(5, 0.0001, "EXP", 0));
	}
	if (boolState[failF_OF_B_op] == true)
	{
		//cout << "6 : xx11_OF_B_op : REPAIR rep  DIST EXP (0.1)  INDUCING boolState[failF_OF_B_op]  =  FALSE" << endl;
		list.push_back(make_tuple(6, 0.1, "EXP", 0));
	}
	if ((boolState[failF_OF_C_fail] == false) && boolState[required_OF_C_fail])
	{
		//cout << "7 : xx10_OF_C_fail : FAULT failF  LABEL \"failure in operation C_fail\"  DIST EXP (0.0001)  INDUCING boolState[failF_OF_C_fail]  =  TRUE" << endl;
		list.push_back(make_tuple(7, 0.0001, "EXP", 0));
	}
	if (boolState[failF_OF_C_fail] == true)
	{
		//cout << "8 : xx11_OF_C_fail : REPAIR rep  DIST EXP (0.1)  INDUCING boolState[failF_OF_C_fail]  =  FALSE" << endl;
		list.push_back(make_tuple(8, 0.1, "EXP", 0));
	}
	if (boolState[failI_OF_C_start] == true)
	{
		//cout << "11 : xx24_OF_C_start : REPAIR rep  DIST EXP (0.1)  INDUCING boolState[failI_OF_C_start]  =  FALSE" << endl;
		list.push_back(make_tuple(11, 0.1, "EXP", 0));
	}
	if ((boolState[failF_OF_P_op] == false) && boolState[required_OF_P_op])
	{
		//cout << "12 : xx10_OF_P_op : FAULT failF  LABEL \"failure in operation P_op\"  DIST EXP (0.0001)  INDUCING boolState[failF_OF_P_op]  =  TRUE" << endl;
		list.push_back(make_tuple(12, 0.0001, "EXP", 0));
	}
	if (boolState[failF_OF_P_op] == true)
	{
		//cout << "13 : xx11_OF_P_op : REPAIR rep  DIST EXP (0.1)  INDUCING boolState[failF_OF_P_op]  =  FALSE" << endl;
...
void storm::figaro::FigaroProgram_BDMP_02_3trains_standby_redundancy_No_trim_repair::runOnceInteractionStep_initialization()
{
	if (boolState[failF_OF_A_op] == true )
	{
...
void storm::figaro::FigaroProgram_BDMP_02_3trains_standby_redundancy_No_trim_repair::runOnceInteractionStep_propagate_effect_S()
{
...
void storm::figaro::FigaroProgram_BDMP_02_3trains_standby_redundancy_No_trim_repair::runOnceInteractionStep_propagate_effect_required()
{
	...
	if ((boolState[relevant_evt_OF_OR_2] && ( !boolState[S_OF_OR_2])) || (boolState[relevant_evt_OF_C_start] && ( !boolState[S_OF_C_start])) )
	{
	...
	if (((boolState[relevant_evt_OF_AND_1] && ( !boolState[S_OF_AND_1])) || (  boolState[relevant_evt_OF_OR_2] && ( !boolState[S_OF_OR_2]))) || (boolState[relevant_evt_OF_C_fail] && ( !boolState[S_OF_C_fail])) )
	{
...
void storm::figaro::FigaroProgram_BDMP_02_3trains_standby_redundancy_No_trim_repair::runOnceInteractionStep_propagate_leaves()
{
...
void storm::figaro::FigaroProgram_BDMP_02_3trains_standby_redundancy_No_trim_repair::runOnceInteractionStep_tops()
{
...
void
storm::figaro::FigaroProgram_BDMP_02_3trains_standby_redundancy_No_trim_repair::runInteractions() {
    int counter = 0;
        int comparator = 1;
        doReinitialisations();
	...
	boolFailureState[exp0] = ( boolState[S_OF_UE_1] );
        //cout << endl;
    }void storm::figaro::FigaroProgram_BDMP_02_3trains_standby_redundancy_No_trim_repair::printstatetuple(){
        ...
        int_fast64_t storm::figaro::FigaroProgram_BDMP_02_3trains_standby_redundancy_No_trim_repair::stateSize() const{
            return numBoolState;
}
        void storm::figaro::FigaroProgram_BDMP_02_3trains_standby_redundancy_No_trim_repair::fireinsttransitiongroup(std::string user_input_ins)
