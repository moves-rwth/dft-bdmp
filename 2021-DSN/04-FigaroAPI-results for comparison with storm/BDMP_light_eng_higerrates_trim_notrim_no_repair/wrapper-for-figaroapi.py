...
##input_file_path = "./figaro_files/03_trim_repair.fi"
...
for terminal_path in lowest_dirs:
    ...
    input_file_path = os.path.join(current_directory, terminal_path, "export_fig0.fi")
    cpp_file_path = os.path.join(current_directory, terminal_path)

# input_file_path = "./figaro_files/01-2trainsElec/No_trim_No_repair/export_fig0.fi"
    ...
    figaro_occurrence_pattern = re.compile('\s*(.*?)\s*GROUP\s*(.*?)\s*IF\s*(.*?)\s*MAY_OCCUR\s*(.*?)\s*;',
                                           flags=re.DOTALL | re.UNICODE)
    figaro_may_occur_pattern = re.compile('(?=FAULT|REPAIR)\s*(.*?)\s*DIST\s*(.*?)\s*INDUCING\s*(.*?)\s*\Z')
    figaro_INS_pattern = re.compile(
        "(?=FAULT|TRANSITION)\s*(.*?)\s*(?=LABEL|\s)\s*(.*?)\s*(?=DIST)\s*(.*?)\s*(?=INDUCING)\s*(.*?)\s*\Z")
    ...
    declared_steps = re.findall(r"STEPS_ORDER\s*(.*?)\s*GROUP_NAMES", figaro_code, flags=re.DOTALL | re.UNICODE)[0]
    ...
    for om in objects_matches[:, 2]:
        if om.find('OBJECT ', ) >= 0 and om.find('OBJECT ', ) <= 5:
            ...
            om_count += 1
            om_tmp = om
            objects_matches = np.insert(objects_matches, tmp_loop_count, np.array(figaro_objects_pattern.findall(om)),
                                        axis=0)
    ...
    for object_match in objects_matches:
        ...
        if object_type == "undes_event":
            cpp_top_event = "S_OF_" + object_name
        ...
        for member_match in members_matches:
            ...
            if member_match[0] == 'CONSTANT':
                        ...
                        for i in attribute[1:]:
                            ...
                            if attribute_identifier == 'LABEL':
                                ...
                                # for the enumerated variables
                                    ...
                                    cpp_declaration_attributes += i.replace("\n", "").replace("\t", "")
                                #                              i = i.replace('REAL','')
                                #                              i = change_boolean(i)
                                ...
                                attribute_cpp_line += currentIndentation + 'bool ' + i[
                                    0] + '_OF_' + attribute_name + '_OF_' + object_name + ' = ' + i[1] + ";\n"
                                cpp_declaration_attributes += 'bool ' + i[
                                    0] + '_OF_' + attribute_name + '_OF_' + object_name + ';\n'
                                cpp_all_reinitialisations += (
                                            str(i[0]) + '_OF_' + attribute_name + '_OF_' + object_name + ';\n')
                ...
                for occurrence in occurrences:
                    ...
                    occurrence_name = occurrence[0]
                    occurrence_group = occurrence[1]
                    ...
                    if (occurrence_may_occur.find(" INS ") >= 0):
                        ...
                        occurrence_cpp_line += occurrence_if
                        """ """
                        occurrence_comment = "cout << \"" + str(
                            occurrence_count) + " : " + occurrence_name + "_OF_" + object_name + " :" \
                                                                                                 " " + str(
                            occurrence_may_occur.replace('\n\t\t', ' | ').replace('"', '\\"')) + "\" <<endl; \n"
                        ...
                        may_occur_cpp = change_boolean(may_occur_cpp)
    ...
    for i in list(set(all_constants)):  # don't take into account duplicates
        ...
        constant_cpp_line = currentIndentation + interaction_domain + ' const ' + i[0] + '_OF_' + i[3] + ' = ' + i[
            2].replace('\'', '"') + ';'
    ...
    cppSourceFile = open(os.path.join(current_directory, terminal_path, "FigaroModel" + str(count) + ".cpp"), "w")
    cppSourceFile.write("""
    ...
    namespace storm{
        namespace bdmp{
            ...
            storm::bdmp::FigaroProgram::FigaroProgram()
            {
    ...
    def replaceByBoolState(expression):
        """
        Replace the attribute in 'expression' by 'boolState['+attribute+']'.
        ...
        """
        for bs in list_boolState:
            if expression.find(bs) >= 0:
                expression = expression.replace('(' + bs + ' ', "(boolState[" + bs + "] ")
                expression = expression.replace(' ' + bs + ')', " boolState[" + bs + "])")
                expression = expression.replace('(' + bs + ')', "(boolState[" + bs + "])")
                expression = expression.replace(' ' + bs + ' ', " boolState[" + bs + "] ")
                expression = expression.replace("			" + bs + ' ', "			boolState[" + bs + "] ")
                ...
                expression = expression.replace('&&' + bs + ' ', "&& boolState[" + bs + "] ")
                expression = expression.replace('&&' + bs + ')', "&& boolState[" + bs + "])")
                expression = expression.replace('(' + bs + '\n', "(boolState[" + bs + "]\n")
                expression = expression.replace(' ' + bs + '&&', " boolState[" + bs + "] &&")
                expression = expression.replace('(' + bs + ',', "(boolState[" + bs + "],")
                expression = expression.replace(' ' + bs + ',', " boolState[" + bs + "],")
                expression = expression.replace(' ' + bs + ';', " boolState[" + bs + "];")
    ...
    cpp_boolState = ""
    cpp_map_boolState = "            std::map<std::string, size_t> mFigaroelementindex = {"
    ...
    for att in re.findall(r'\n\s*(.*?)\s*;', cpp_declaration_attributes):
        if att.find("REINITIALISATION_OF_") < 0:
            ...
            if att.find("bool") >= 0:
                ...
                cpp_map_boolState += "\n            \t{\"" + att.replace("bool ", "") + "\" , " + str(
                    index_boolState) + "},"

                if (att.replace("bool ", "").startswith("S_OF_")):
                    cpp_map_failureIndex += "\n            \t{\"" + att.replace("bool ", "") + "\" , " + str(
                        index_boolState) + "},"

                index_boolState += 1
            else:

                #            raise ValueError("Only boolean attributes in the Figaro files are handled for this version of the parser.")
                print(" !!! Not boolean variable : ", att)
                # TODO
                # mettre un commentaire brut de la trace du fichier figaro
        else:
    ...
    indent = '\n\t'
    currentEdition = "\nvoid storm::bdmp::FigaroProgram::init()\n{"
    # {
    currentEdition += indent + "cout <<\">>>>>>>>>>>>>>>>>>>> Initialization of variables <<<<<<<<<<<<<<<<<<<<<<<\" << endl;\n"
    cpp_all_attributes_list = re.findall(r"\n\s*(.*?)\s*=\s*(.*?)\s*;",
                                         cpp_all_attributes)  # ne prend pas les valeurs non assigné lors de l'initialisation ainsi
    ...
    currentEdition = "\n\nvoid storm::bdmp::FigaroProgram::saveCurrentState()\n{\n\t// cout <<\">>>>>>>>>>>>>>>>>>>> Saving current state  <<<<<<<<<<<<<<<<<<<<<<<\" << endl;"
    ...
    currentEdition = "\n\nint storm::bdmp::FigaroProgram::compareStates()\n{"
    indent = '\n\t'
    # {
    currentEdition += indent + "// cout <<\">>>>>>>>>>>>>>>>>>>> Comparing state with previous one (return number of differences) <<<<<<<<<<<<<<<<<<<<<<<\" << endl;\n"

    # v0.3
    currentEdition += indent + "return backupBoolState != boolState; "
    currentEdition += ("\n}")
    ...
    currentEdition = "\n\nvoid storm::bdmp::FigaroProgram::printState()\n{"
    currentEdition += indent + "cout <<\"\\n==================== Print of the current state :  ====================\" << endl;\n"
    for att in attribute_list:
        if att.startswith("REINITIALISATION_OF_") == False:
            currentEdition += "\n\tcout << \"Attribute :  {} | Value : \" << boolState[{}] << endl;".format(att, att)
    currentEdition += ("\n}")
    cppSourceFile.write(currentEdition)

    ### checking if figaro model has inst transitions
    currentEdition = "\n\nbool storm::bdmp::FigaroProgram::figaromodelhasinstransitions()\n{"
    ...
    ### Reinitializatrion function declaration
    currentEdition = "\n\nvoid storm::bdmp::FigaroProgram::doReinitialisations()\n{"
    indent = '\n\t'
    ...
    currentEdition = "\n\nvoid storm::bdmp::FigaroProgram::fireOccurrence(int numFire)\n{"
    # {
    indent = '\n\t'
    ...
    # }
    currentEdition = replaceByBoolState(currentEdition)  # v0.3
    cppSourceFile.write(currentEdition)
    ...
    for ocl in raw_occurrences_conditions_list:
        if ocl.startswith('FIRE') == False:
            ocl = ocl.replace('(', '').replace(')', '')
            occurrences_conditions_list.append(ocl)

    currentEdition = ""
    currentEdition_top = "std::vector<std::tuple<int, double, std::string, int>> storm::bdmp::FigaroProgram::showFireableOccurrences()\n{"
    # {
    indent = '\n\t'
    ...
    currentEdition = replaceByBoolState(currentEdition)
    ...
    for s in ordered_interaction_step_name:
        if len(cpp_interractions_separated_steps[step_names.index(s)]) > 0:
            currentEdition = ""
            indent = '\n\t'
            interaction_function = "\nvoid storm::bdmp::FigaroProgram::runOnceInteractionStep_" + s + "()"
    ...
    currentEdition = """void storm::bdmp::FigaroProgram::runInteractions() {
        int counter = 0;
        int comparator = 1;
        doReinitialisations();
    ...
    for ifn in interaction_function_names:
        ...
        currentEdition += ifn.replace("void storm::bdmp::FigaroProgram::", "\t\t") + ";"
        ...
        if (comparator <= 0)
        {
            ...
            "\nvoid storm::bdmp::FigaroProgram::", "").replace("nOnceI", "nI") + """ ." << endl;
        }
        else {
            cout << "==> Maximum of interactions loop  reached : #" << counter <<" for """ + ifn.replace(
            "\nvoid storm::bdmp::FigaroProgram::", "") + """." << endl;
        }
         """
    ...
    cppSourceFile.write("""
            void storm::bdmp::FigaroProgram::printstatetuple(){
                std::cout<<"\\n State information: (";
                ...
                for (auto const& key: mFigaroelementfailureindex)
                    {
                    std::cout<<boolState.at(key.second);
                    }
                std::cout<<")";
                std::cout<<") : key: (";
                for (auto const& key: mFigaroelementfailureindex)
            ...
            void storm::bdmp::FigaroProgram::fireinsttransitiongroup(std::string user_input_ins)
            {
    ...
    cppHeaderFile = open(os.path.join(current_directory, terminal_path, "FigaroModel" + str(count) + ".h"), "w")
    cppHeaderFile.write("""
    #pragma once
    #include <array>
    #include <map>
    ...
    cppHeaderFile.write("\n" + currentIndentation + "std::string const topevent=\"" + cpp_top_event + "\";")

    cppHeaderFile.write("\n" + currentIndentation + "static int const numBoolState = " + str(numBoolState) + " ;")
    cppHeaderFile.write("\n" + currentIndentation + "std::array<bool, numBoolState> boolState;")
    cppHeaderFile.write("\n" + currentIndentation + "std::array<bool, numBoolState> backupBoolState;")
    cppHeaderFile.write("\n" + currentIndentation + "bool ins_transition_found = false;\n")
    ...
    for ifn in interaction_function_names:
        cppHeaderFile.write(ifn.replace('\n', '\n' + currentIndentation).replace("storm::bdmp::FigaroProgram::", "") + ";")
