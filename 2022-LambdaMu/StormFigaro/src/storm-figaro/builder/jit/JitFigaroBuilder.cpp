    ...
    namespace figaro {
        namespace builder {
            namespace jit {
                ...
                storm::jani::ModelFeatures
                JitFigaroBuilder<ValueType, RewardModelType>::getSupportedJaniFeatures() {
                ...
                template<typename ValueType, typename RewardModelType>
                void  JitFigaroBuilder<ValueType, RewardModelType>::getcreator(boost::function<CreateFunctionType>& creator_dummy)
                {creator_dummy= this->creator;}

                template<typename ValueType, typename RewardModelType>
                bool
                JitFigaroBuilder<ValueType, RewardModelType>::canHandle(storm::jani::Model const &model) {
                    // Check jani features
                ...
                // constructor for Figaro in in time build

                template<typename ValueType, typename RewardModelType>
                JitFigaroBuilder<ValueType, RewardModelType>::JitFigaroBuilder(
                        std::string figaromodel,std::string figaroanalysis, storm::builder::BuilderOptions const &options) :
                        figaroSourceFileName(figaromodel), figaroAnalysisFileName(figaroanalysis), options(options),
                        modelComponentsBuilder(storm::jani::ModelType::CTMC) {

                            //Fix file paths; we should accept both relative and ablsolute paths
                    boost::filesystem::path figaroPath(figaroSourceFileName);
                    boost::filesystem::path xmlPath(figaroAnalysisFileName);

                    if(figaroPath.is_absolute()){
                        figaroSourceFileName = figaroPath.string();

                    }
                    else {
                        figaroSourceFileName = boost::filesystem::current_path().string() +"/"+ figaroSourceFileName;

                    }
                    figaroSourceFilePath = boost::filesystem::path(figaroSourceFileName).parent_path().string();
                    if(xmlPath.is_absolute()){
                        figaroAnalysisFileName = xmlPath.string();
                    }
                    else {
                        figaroAnalysisFileName = boost::filesystem::current_path().string() +"/"+ figaroAnalysisFileName;
                    }
...
#ifdef MACOSX
                        flagStream
                                << "-std=c++14 -stdlib=libc++ -fPIC -march=native -shared -undefined dynamic_lookup ";
#endif
                    ...
                    fig2cppIncludeDirectory = STORM_SOURCE_DIR "/src/storm-figaro/Fig2CPP";
...
//                        parallelAutomata.push_back(this->model.getAutomaton(
//                                topLevelComposition.asAutomatonComposition().getAutomatonName()));
//                    } else {
//                        STORM_LOG_THROW(topLevelComposition.isParallelComposition(),
//                                        storm::exceptions::WrongFormatException, "Expected parallel composition.");
...
//                            STORM_LOG_THROW(composition->isAutomatonComposition(),
//                                            storm::exceptions::WrongFormatException,
//                                            "Expected flat parallel composition.");
//                            parallelAutomata.push_back(
//                                    this->model.getAutomaton(composition->asAutomatonComposition().getAutomatonName()));
//                        }
//                    }
...
//                        this->model.hasUndefinedConstants()) {
//#else
...
//                                        "Model still contains these undefined constants: " + stream.str());
//                    }

//#ifdef STORM_HAVE_CARL
//                    else if (std::is_same<ValueType, storm::RationalFunction>::value &&
//                             !this->model.undefinedConstantsAreGraphPreserving()) {
//                        STORM_LOG_THROW(false, storm::exceptions::InvalidArgumentException,
...
//                                    "The jit model builder does not support the following model feature(s): "
//                                            << features.toString() << ".");
                ...
                template<typename ValueType, typename RewardModelType>
                JitFigaroBuilder<ValueType, RewardModelType>::JitFigaroBuilder(
                        storm::jani::Model const &model, storm::builder::BuilderOptions const &options) : options(
                        options), model(model.substituteConstantsFunctions()), modelComponentsBuilder(
                        model.getModelType()) {
                ...
                JitFigaroBuilder<ValueType, RewardModelType>::execute(std::string command) {
                    ...
                    STORM_LOG_THROW(pipe, storm::exceptions::InvalidStateException,
                                    "Call to popen failed: " << strerror(errno));
                    ...
                    STORM_LOG_TRACE("Executing command took " << std::chrono::duration_cast<std::chrono::milliseconds>(
                            end - start).count() << "ms.");
                ...
                template<typename ValueType, typename RewardModelType>
                boost::filesystem::path JitFigaroBuilder<ValueType, RewardModelType>::writeToTemporaryFile(
                        std::string const &content, std::string const &suffix) {
                    boost::filesystem::path temporaryFile = boost::filesystem::unique_path(
                            "%%%%-%%%%-%%%%-%%%%" + suffix);
                ...
                template<typename ValueType, typename RewardModelType>
                bool JitFigaroBuilder<ValueType, RewardModelType>::checkTemporaryFileWritable() const {
                    bool result = true;
                ...
                template<typename ValueType, typename RewardModelType>
                bool JitFigaroBuilder<ValueType, RewardModelType>::checkCompilerWorks() const {
                    bool result = true;
                    std::string problem =
                            "Unable to compile empty program with C++ compiler. Is the C++ compiler '" + compiler +
                    ...
                    int main() {
                        ...
                        boost::optional<std::string> error = execute(
                                compiler + " " + temporaryFilename + " -o " + outputFilename);

                        boost::filesystem::remove(temporaryFilename);
                        if (error) {
                            result = false;
                            STORM_LOG_ERROR(problem);
                ...
                template<typename ValueType, typename RewardModelType>
                bool JitFigaroBuilder<ValueType, RewardModelType>::checkCompilerFlagsWork() const {
                    bool result = true;
                    std::string problem = "Unable to compile program with the flags '" + compilerFlags +
                                          "'. Does the C++ compiler accept these flags?";
                    try {
                        std::string emptyProgram = R"(
#include <cstdint>
                    ...
                    int main() {
                        ...
                        std::string outputFilename = boost::filesystem::absolute(outputFile).string();
                        boost::optional<std::string> error = execute(
                                compiler + " " + compilerFlags + " " + temporaryFilename + " -o " + outputFilename);
                ...
                template<typename ValueType, typename RewardModelType>
                bool JitFigaroBuilder<ValueType, RewardModelType>::checkBoostAvailable() const {
                    bool result = true;
                    std::string problem = "Unable to compile program using boost. Is boosts's include directory '" +
                                          boostIncludeDirectory + "' set correctly?";
                    try {
...
#include <boost/optional.hpp>
                    ...
                    int main() {
                        ...
                        std::string outputFilename = boost::filesystem::absolute(outputFile).string();
                        boost::optional<std::string> error = execute(
                                compiler + " " + compilerFlags + " " + temporaryFilename + " -I" +
                                boostIncludeDirectory + " -o " + outputFilename);
                ...
                template<typename ValueType, typename RewardModelType>
                bool JitFigaroBuilder<ValueType, RewardModelType>::checkBoostDllAvailable() const {
                    bool result = true;
                    std::string problem =
                            "Unable to compile program using boost's dll library. Is boosts's include directory '" +
                            boostIncludeDirectory + "' pointing to a boost installation with version at least 1.61?";
                    try {
                        std::string program = R"(
#include <boost/dll/alias.hpp>
                ...
                template<typename ValueType, typename RewardModelType>
                bool JitFigaroBuilder<ValueType, RewardModelType>::checkStormHeadersAvailable() const {
                    bool result = true;
                    std::string problem =
                            "Unable to compile program using storm headers. Is storm's include directory '" +
                            stormIncludeDirectory +
...
#include "storm-config.h"
                    ...
                    int main() {
                        ...
                        boost::optional<std::string> error = execute(
                                compiler + " " + compilerFlags + " " + temporaryFilename + " -I" +
                                stormIncludeDirectory + " -o " + outputFilename);
                ...
                template<typename ValueType, typename RewardModelType>
                bool JitFigaroBuilder<ValueType, RewardModelType>::checkCarlAvailable() const {
                    bool result = true;
                    std::string problem =
                            "Unable to compile program using Carl data structures. Is Carls's include directory '" +
                            carlIncludeDirectory + "' set correctly?";
                    try {
                        std::string program = R"(
#include "storm/adapters/RationalFunctionAdapter.h"
                    ...
                    int main() {
                        ...
                        std::string outputFilename = boost::filesystem::absolute(outputFile).string();
                        std::string includes = "";
                        for (std::string const &dir : {stormIncludeDirectory, carlIncludeDirectory, clnIncludeDirectory,
                                                       gmpIncludeDirectory}) {
                            if (dir != "") {
                                includes += " -I" + dir;
                            }
                        ...
                        boost::optional<std::string> error = execute(
                                compiler + " " + compilerFlags + " " + temporaryFilename + includes + " -o " +
                                outputFilename);
                ...
                template<typename ValueType, typename RewardModelType>
                bool JitFigaroBuilder<ValueType, RewardModelType>::doctor() const {
                    bool result = true;
                ...
                template<typename ValueType, typename RewardModelType>
                std::shared_ptr<storm::figaro::FigaroProgram>
                JitFigaroBuilder<ValueType, RewardModelType>::build() {
                    // (0) convert Figaro file to Cpp/H file

#ifdef false
    std::string command =
            "python " + fig2cppIncludeDirectory + "/Fig2Cpp_Alexis.py -fig0 " + figaroSourceFileName + " -a " +
            figaroAnalysisFileName + " -cpp " + fig2cppIncludeDirectory + "/FigaroModel.cpp" + " -hpp " +
            fig2cppIncludeDirectory + "/FigaroModel.h";

    std::system(command.c_str());


    // (1) generate dynamic library
    boost::filesystem::path dynamicLibraryPath = compileToSharedLibrary(fig2cppIncludeDirectory + "/FigaroModel.h",
                                                                        fig2cppIncludeDirectory + "/FigaroModel.cpp");
#endif

                    std::string command =
                            "python3 " + fig2cppIncludeDirectory + "/Fig2Cpp_Alexis.py -fig0 " + figaroSourceFileName + " -a " +
                            figaroAnalysisFileName + " -cpp " + figaroSourceFilePath + "/FigaroModel.cpp" + " -hpp " +
                                    figaroSourceFilePath + "/FigaroModel.h";

                    std::system(command.c_str());

                    // (1) generate dynamic library
                    boost::filesystem::path dynamicLibraryPath = compileToSharedLibrary(figaroSourceFilePath + "/FigaroModel.h",
                                                                                        figaroSourceFilePath + "/FigaroModel.cpp");
...
//                    (2) Link this dynamic library to figarotemplate
                    auto figaroProgram = createFigaroModel(dynamicLibraryPath);

                    return figaroProgram;
...
//                                        "Could not create the source code for model generation (error: " << e.what()
//                                                                                                         << ").");
//                    }
...
//                    boost::filesystem::path dynamicLibraryPath = compileToSharedLibrary(temporarySourceFile, temporarySourceFile);
//                    STORM_LOG_TRACE("Successfully compiled shared library.");
//
...
//                    auto start = std::chrono::high_resolution_clock::now();
//
//                    std::shared_ptr<storm::models::sparse::Model<ValueType, storm::models::sparse::StandardRewardModel<ValueType>>> sparseModel(
//                            nullptr);
//                    boost::optional<std::string> error;
//                    try {
////                        sparseModel = std::shared_ptr<storm::models::sparse::Model<ValueType, storm::models::sparse::StandardRewardModel<ValueType>>>(
////                                builder->build());
////                        STORM_LOG_THROW(sparseModel, storm::exceptions::UnexpectedException,
...
//
//                    auto end = std::chrono::high_resolution_clock::now();
//                    STORM_LOG_TRACE("Building model took " << std::chrono::duration_cast<std::chrono::milliseconds>(
//                            end - start).count() << "ms.");
//
//                    // (7) Delete the shared library.
//                    boost::filesystem::remove(dynamicLibraryPath);
//
//                    STORM_LOG_THROW(!error, storm::exceptions::WrongFormatException,
//                                    "Model building failed. Reason: " << error.get());
//
...
//                    return sparseModel;
                ...
                template<typename ValueType, typename RewardModelType>
                cpptempl::data_map JitFigaroBuilder<ValueType, RewardModelType>::generateModelData() {
                    ...
                    generateRewards(
                            modelData); // We need to generate the reward information before the edges, because we already use it there.
                ...
                template<typename ValueType, typename RewardModelType>
                cpptempl::data_map JitFigaroBuilder<ValueType, RewardModelType>::generateBooleanVariable(
                        storm::jani::BooleanVariable const &variable) {
                ...
                template<typename ValueType, typename RewardModelType>
                cpptempl::data_map
                JitFigaroBuilder<ValueType, RewardModelType>::generateBoundedIntegerVariable(
                        storm::jani::BoundedIntegerVariable const &variable) {
                    cpptempl::data_map result;
                    ...
                    if (lowerBound != 0) {
                        lowerBoundShiftSubstitution[variable.getExpressionVariable()] =
                                variable.getExpressionVariable() - model.getManager().integer(-lowerBound);
                    }
                ...
                template<typename ValueType, typename RewardModelType>
                cpptempl::data_map
                JitFigaroBuilder<ValueType, RewardModelType>::generateUnboundedIntegerVariable(
                        storm::jani::UnboundedIntegerVariable const &variable) {
                    cpptempl::data_map result;
                ...
                template<typename ValueType, typename RewardModelType>
                cpptempl::data_map JitFigaroBuilder<ValueType, RewardModelType>::generateRealVariable(
                        storm::jani::RealVariable const &variable) {
                    cpptempl::data_map result;
                    ...
                    if (variable.hasInitExpression()) {
                        if (std::is_same<double, ValueType>::value) {
                            result["init"] = expressionTranslator.translate(variable.getInitExpression(),
                                                                            storm::expressions::ToCppTranslationOptions(
                                                                                    variablePrefixes, variableToName,
                                                                                    storm::expressions::ToCppTranslationMode::CastDouble));
                        } else if (std::is_same<storm::RationalNumber, ValueType>::value) {
                            result["init"] = expressionTranslator.translate(variable.getInitExpression(),
                                                                            storm::expressions::ToCppTranslationOptions(
                                                                                    variablePrefixes, variableToName,
                                                                                    storm::expressions::ToCppTranslationMode::CastRationalNumber));
                        } else if (std::is_same<storm::RationalFunction, ValueType>::value) {
                            result["init"] = expressionTranslator.translate(variable.getInitExpression(),
                                                                            storm::expressions::ToCppTranslationOptions(
                                                                                    variablePrefixes, variableToName,
                                                                                    storm::expressions::ToCppTranslationMode::CastRationalFunction));
                        }
                    }
                ...
                template<typename ValueType, typename RewardModelType>
                cpptempl::data_map JitFigaroBuilder<ValueType, RewardModelType>::generateLocationVariable(
                        storm::jani::Automaton const &automaton) {
                    cpptempl::data_map result;

                    result["name"] = registerVariable(getLocationVariable(automaton), false);
                    result["numberOfBits"] = static_cast<uint64_t>(std::ceil(
                            std::log2(automaton.getNumberOfLocations())));

                    return result;
                }

                template<typename ValueType, typename RewardModelType>
                void JitFigaroBuilder<ValueType, RewardModelType>::generateVariables(
                        cpptempl::data_map &modelData) {
                    ...
                    for (auto const &variable : model.getGlobalVariables().getBoundedIntegerVariables()) {
                        cpptempl::data_map newBoundedIntegerVariable = generateBoundedIntegerVariable(
                                variable.asBoundedIntegerVariable());
                        if (variable.isTransient()) {
                    ...
                    for (auto const &variable : model.getGlobalVariables().getUnboundedIntegerVariables()) {
                        cpptempl::data_map newUnboundedIntegerVariable = generateUnboundedIntegerVariable(
                                variable.asUnboundedIntegerVariable());
                        if (variable.isTransient()) {
                    ...
                    for (auto const &automatonRef : parallelAutomata) {
                        ...
                        for (auto const &variable : automaton.getVariables().getBooleanVariables()) {
                            cpptempl::data_map newBooleanVariable = generateBooleanVariable(
                                    variable.asBooleanVariable());
                            if (variable.isTransient()) {
                                transientBooleanVariables.push_back(newBooleanVariable);
                            } else {
                                nonTransientBooleanVariables.push_back(newBooleanVariable);
                    ...
                    nonTransientVariables["unboundedInteger"] = cpptempl::make_data(
                            nonTransientUnboundedIntegerVariables);
                ...
                template<typename ValueType, typename RewardModelType>
                void JitFigaroBuilder<ValueType, RewardModelType>::generateInitialStates(
                        cpptempl::data_map &modelData) {
                    ...
                    while (solver->check() == storm::solver::SmtSolver::CheckResult::Sat) {
                        ...
                        for (auto const &variable : this->model.getGlobalVariables().getBooleanVariables()) {
                            ...
                            storm::expressions::Expression localBlockingExpression = variableValue ? !expressionVariable
                                                                                                   : expressionVariable;
                            blockingExpression = blockingExpression.isInitialized() ? blockingExpression ||
                                                                                      localBlockingExpression
                                                                                    : localBlockingExpression;
                        }
                        for (auto const &variable : this->model.getGlobalVariables().getBoundedIntegerVariables()) {
                            ...
                            storm::expressions::Expression localBlockingExpression =
                                    expressionVariable != model->getManager().integer(variableValue);
                            blockingExpression = blockingExpression.isInitialized() ? blockingExpression ||
                                                                                      localBlockingExpression
                                                                                    : localBlockingExpression;
                        }
                        for (auto const &automatonRef : parallelAutomata) {
                            ...
                            for (auto const &variable : automaton.getVariables().getBooleanVariables()) {
                                ...
                                storm::expressions::Expression localBlockingExpression = variableValue
                                                                                         ? !expressionVariable
                                                                                         : expressionVariable;
                                blockingExpression = blockingExpression.isInitialized() ? blockingExpression ||
                                                                                          localBlockingExpression
                        ...
                        while (true) {
                            ...
                            for (uint64_t index = 0; index < initialLocationsIterators.size(); ++index) {
                                ...
                                if (automaton.getNumberOfLocations() > 1) {
                                    completeAssignment.push_back(
                                            generateLocationAssignment(automaton, *initialLocationsIterators[index]));
                                }
                            }
                            ...
                            for (; index < initialLocationsIterators.size(); ++index) {
                                ...
                                if (initialLocationsIterators[index] ==
                                    parallelAutomata[index].get().getInitialLocationIndices().cend()) {
                ...
                template<typename ValueType, typename RewardModelType>
                void JitFigaroBuilder<ValueType, RewardModelType>::generateRewards(
                        cpptempl::data_map &modelData) {
                    ...
                    if (this->options.isBuildAllRewardModelsSet()) {
                        for (auto const &rewExpr : model.getAllRewardModelExpressions()) {
                            STORM_LOG_ERROR_COND(rewExpr.second.isVariable(),
                                                 "The jit builder can not build the non-trivial reward expression '"
                                                         << rewExpr.second << "'.");
                            rewardVariables.push_back(
                                    rewExpr.second.getBaseExpression().asVariableExpression().getVariable());
                        }
                    } else {
                        for (auto const &rewardModelName : this->options.getRewardModelNames()) {
                            ...
                            STORM_LOG_ERROR_COND(rewExpr.isVariable(),
                                                 "The jit builder can not build the non-trivial reward expression '"
                                                         << rewExpr << "'.");
                    ...
                    for (auto const &variable : rewardVariables) {
                        ...
                        rewardModels.emplace_back(variable.getName(), hasLocationRewards,
                                                  hasEdgeRewards || hasDestinationRewards, false);
                ...
                template<typename ValueType, typename RewardModelType>
                cpptempl::data_map
                JitFigaroBuilder<ValueType, RewardModelType>::generateSynchronizationVector(
                        cpptempl::data_map &modelData, storm::jani::ParallelComposition const &parallelComposition,
                    ...
                    for (auto const &inputActionName : synchronizationVector.getInput()) {
                        if (!storm::jani::SynchronizationVector::isNoActionInput(inputActionName)) {
                            storm::jani::Automaton const &automaton = model.getAutomaton(
                                    parallelComposition.getSubcomposition(
                                            position).asAutomatonComposition().getAutomatonName());
                                    ...
                                    for (auto const &destination : edge.getDestinations()) {
                                        if (!destination.getOrderedAssignments().empty()) {
                                            if (firstDestination) {
                                                ...
                                                lowestLevel = std::min(lowestLevel,
                                                                       destination.getOrderedAssignments().getLowestLevel());
                                                highestLevel = std::max(highestLevel,
                    ...
                    indent(vectorSource, indentLevel - 4) << "void performSynchronizedDestinations_"
                    ...
                    if (generateLevelCode) {
                        indent(vectorSource, indentLevel + 1)
                                << "for (int64_t level = lowestLevel; level <= highestLevel; ++level) {" << std::endl;
                        ++indentLevel;
                    }
                    for (uint64_t index = 0; index < numberOfActionInputs; ++index) {
                    ...
                    indent(vectorSource, indentLevel + 1)
                            << "IndexType outStateIndex = getOrAddIndex(out, statesToExplore);" << std::endl;
                    ...
                    indent(vectorSource, indentLevel) << "void performSynchronizedDestinations_"
                                                      << synchronizationVectorIndex
                                                      << "(StateType const& in, StateBehaviour<IndexType, ValueType>& behaviour, StateSet<StateType>& statesToExplore, ";
                    for (uint64_t index = 0; index < numberOfActionInputs; ++index) {
                    ...
                    for (uint64_t index = 0; index < numberOfActionInputs; ++index) {
                        indent(vectorSource, indentLevel + 1 + index) << "for (auto const& destination" << index
                                                                      << " : edge" << index << ") {" << std::endl;
                        if (generateLevelCode) {
                            if (index == 0) {
                                indent(vectorSource, indentLevel + 2 + index) << "lowestLevel = destination" << index
                                                                              << ".lowestLevel();" << std::endl;
                                indent(vectorSource, indentLevel + 2 + index) << "highestLevel = destination" << index
                                                                              << ".highestLevel();" << std::endl;
                            } else {
                                indent(vectorSource, indentLevel + 2 + index)
                                        << "lowestLevel = std::min(lowestLevel, destination" << index
                                        << ".lowestLevel());" << std::endl;
                                indent(vectorSource, indentLevel + 2 + index)
                                        << "highestLevel = std::max(highestLevel, destination" << index
                                        << ".highestLevel());" << std::endl;
                            }
                        }
                    }
                    ...
                    for (uint64_t index = 0; index < numberOfActionInputs; ++index) {
                        indent(vectorSource, indentLevel) << "void performSynchronizedEdges_"
                                                          << synchronizationVectorIndex << "_" << index
                                                          << "(StateType const& in, std::vector<std::vector<std::reference_wrapper<Edge const>>> const& edges,  StateBehaviour<IndexType, ValueType>& behaviour, StateSet<StateType>& statesToExplore";
                        if (index > 0) {
                            vectorSource << ", ";
                        }
                        ...
                        indent(vectorSource, indentLevel + 1) << "for (auto const& edge" << index << " : edges["
                                                              << index << "]) {" << std::endl;
                        indent(vectorSource, indentLevel + 2) << "edge" << index
                                                              << ".get().perform(in, transientIn, transientOut";
                        if (options.isExplorationChecksSet()) {
                            vectorSource << ", variableWrites";
                        }
                        vectorSource << ");" << std::endl;
                        if (index + 1 < numberOfActionInputs) {
                            indent(vectorSource, indentLevel + 2) << "performSynchronizedEdges_"
                                                                  << synchronizationVectorIndex << "_" << (index + 1)
                            ...
                            indent(vectorSource, indentLevel + 2)
                                    << "Choice<IndexType, ValueType>& choice = behaviour.addChoice();" << std::endl;

                            std::stringstream tmp;
                            indent(tmp, indentLevel + 2) << "choice.resizeRewards({$edge_destination_rewards_count});"
                                                         << std::endl;
                            indent(tmp, indentLevel + 2)
                                    << "{% for reward in edge_rewards %}choice.addReward({$reward.index}, transientOut.{$reward.variable});"
                                    << std::endl;
                            indent(tmp, indentLevel + 2) << "{% endfor %}" << std::endl;

                            vectorSource << cpptempl::parse(tmp.str(), modelData);

                            indent(vectorSource, indentLevel + 2) << "performSynchronizedDestinations_"
                                                                  << synchronizationVectorIndex
                                                                  << "(in, behaviour, statesToExplore, ";
                            for (uint64_t innerIndex = 0; innerIndex <= index; ++innerIndex) {
                                vectorSource << "edge" << innerIndex << ", ";
                    ...
                    indent(vectorSource, indentLevel) << "void get_edges_" << synchronizationVectorIndex
                        ...
                        if (!storm::jani::SynchronizationVector::isNoActionInput(inputActionName)) {
                            ...
                            for (auto const &edge : automaton.getEdges()) {
                                if (edge.getActionIndex() == actionIndex) {
                                    ...
                                    indent(vectorSource, indentLevel + 2) << "if (edge_enabled_" << edgeName
                                                                          << "(state, transientIn)) {" << std::endl;
                    ...
                    indent(vectorSource, indentLevel) << "void exploreSynchronizationVector_"
                                                      << synchronizationVectorIndex
                                                      << "(StateType const& state, TransientVariables const& transientIn, StateBehaviour<IndexType, ValueType>& behaviour, StateSet<StateType>& statesToExplore) {"
                                                      << std::endl;
                    indent(vectorSource, indentLevel + 1) << "#ifndef NDEBUG" << std::endl;
                    indent(vectorSource, indentLevel + 1) << "std::cout << \"Exploring synchronization vector "
                                                          << synchronizationVectorIndex << ".\" << std::endl;"
                                                          << std::endl;
                    indent(vectorSource, indentLevel + 1) << "#endif" << std::endl;
                    indent(vectorSource, indentLevel + 1)
                            << "std::vector<std::vector<std::reference_wrapper<Edge const>>> edges("
                    ...
                    for (auto const &input : synchronizationVector.getInput()) {
                        if (!storm::jani::SynchronizationVector::isNoActionInput(input)) {
                            indent(vectorSource, indentLevel + 1) << "get_edges_" << synchronizationVectorIndex
                                                                  << "(state, transientIn, edges["
                                                                  << participatingPosition << "], "
                                                                  << participatingPosition << ");" << std::endl;
                            indent(vectorSource, indentLevel + 1) << "if (edges[" << participatingPosition
                                                                  << "].empty()) {" << std::endl;
                ...
                template<typename ValueType, typename RewardModelType>
                void
                JitFigaroBuilder<ValueType, RewardModelType>::generateEdges(cpptempl::data_map &modelData) {
                    ...
                    if (topLevelComposition.isAutomatonComposition()) {
                        ...
                        std::vector<std::set<uint64_t>> synchronizingActions(
                                parallelComposition.getNumberOfSubcompositions());
                        ...
                        for (auto const &synchronizationVector : parallelComposition.getSynchronizationVectors()) {
                            ...
                            if (createVector) {
                                cpptempl::data_map vector = generateSynchronizationVector(modelData,
                                                                                          parallelComposition,
                        ...
                        for (auto const &composition : parallelComposition.getSubcompositions()) {
                            storm::jani::Automaton const &automaton = model.getAutomaton(
                                    composition->asAutomatonComposition().getAutomatonName());

                            // Add all edges with an action index that is not mentioned in any synchronization vector as
                            // non-synchronizing edges.
                            uint64_t edgeIndex = 0;
                            for (auto const &edge : automaton.getEdges()) {
                                if (synchronizingActions[position].find(edge.getActionIndex()) !=
                                    synchronizingActions[position].end()) {
                ...
                template<typename ValueType, typename RewardModelType>
                cpptempl::data_map JitFigaroBuilder<ValueType, RewardModelType>::generateEdge(
                        storm::jani::Automaton const &automaton, uint64_t edgeIndex, storm::jani::Edge const &edge) {
                    ...
                    if (automaton.getNumberOfLocations() > 1) {
                        edgeData["guard"] = expressionTranslator.translate(
                                shiftVariablesWrtLowerBound(edge.getGuard()) && getLocationVariable(automaton) ==
                                                                                model.getManager().integer(
                                                                                        edge.getSourceLocationIndex()),
                    ...
                    edgeData["transient_variables_in_destinations"] = cpptempl::make_data(
                            transientVariablesInDestinationsData);
                ...
                template<typename ValueType, typename RewardModelType>
                cpptempl::data_map JitFigaroBuilder<ValueType, RewardModelType>::generateDestination(
                        storm::jani::Automaton const &automaton, uint64_t destinationIndex,
                        storm::jani::EdgeDestination const &destination) {
                    cpptempl::data_map destinationData;

                    cpptempl::data_list levels = generateLevels(automaton, destination.getLocationIndex(),
                                                                destination.getOrderedAssignments());
                    destinationData["name"] = asString(destinationIndex);
                    destinationData["levels"] = cpptempl::make_data(levels);
                    storm::expressions::Expression shiftedProbabilityExpression = shiftVariablesWrtLowerBound(
                            destination.getProbability());
                    if (std::is_same<double, ValueType>::value) {
                    ...
                    if (destination.getOrderedAssignments().empty()) {
                        ...
                        destinationData["highestLevel"] = asString(
                                destination.getOrderedAssignments().getHighestLevel());
                    }
                ...
                template<typename ValueType, typename RewardModelType>
                cpptempl::data_list JitFigaroBuilder<ValueType, RewardModelType>::generateLevels(
                        storm::jani::Automaton const &automaton, uint64_t destinationLocationIndex,
                    ...
                    if (!assignments.empty()) {
                        ...
                        cpptempl::data_map level;
                        if (automaton.getNumberOfLocations() > 1) {
                            nonTransientAssignmentData.push_back(
                                    generateLocationAssignment(automaton, destinationLocationIndex));
                        }
                        level["non_transient_assignments"] = cpptempl::make_data(nonTransientAssignmentData);
                        level["transient_assignments"] = cpptempl::make_data(transientAssignmentData);
                        level["index"] = asString(currentLevel);
                ...
                cpptempl::data_map JitFigaroBuilder<ValueType, RewardModelType>::generateAssignment(
                        storm::jani::Variable const &variable, ValueTypePrime value) const {
                    cpptempl::data_map result;
                ...
                template<typename ValueType, typename RewardModelType>
                cpptempl::data_map JitFigaroBuilder<ValueType, RewardModelType>::generateLocationAssignment(
                        storm::jani::Automaton const &automaton, uint64_t value) const {
                    cpptempl::data_map result;
                ...
                cpptempl::data_map JitFigaroBuilder<ValueType, RewardModelType>::generateAssignment(
                    ...
                    if (realVariables.find(assignment.getExpressionVariable()) != realVariables.end()) {
                        if (std::is_same<double, ValueType>::value) {
                            options = storm::expressions::ToCppTranslationOptions(variablePrefixes, variableToName,
                                                                                  storm::expressions::ToCppTranslationMode::CastDouble);
                        } else if (std::is_same<storm::RationalNumber, ValueType>::value) {
                            options = storm::expressions::ToCppTranslationOptions(variablePrefixes, variableToName,
                                                                                  storm::expressions::ToCppTranslationMode::CastRationalNumber);
                    ...
                    if (lowerBoundIt != lowerBounds.end()) {
                        result["value"] = expressionTranslator.translate(
                                shiftVariablesWrtLowerBound(assignment.getAssignedExpression()) +
                                model.getManager().integer(-lowerBoundIt->second), options);
                    } else {
                        result["value"] = expressionTranslator.translate(
                                shiftVariablesWrtLowerBound(assignment.getAssignedExpression()), options);
                    }
                    return result;
                }

                template<typename ValueType, typename RewardModelType>
                void JitFigaroBuilder<ValueType, RewardModelType>::generateLocations(
                        cpptempl::data_map &modelData) {
                    ...
                    for (auto const &automatonRef : parallelAutomata) {
                        ...
                        for (auto const &location : automaton.getLocations()) {
                            ...
                            if (automaton.getNumberOfLocations() > 1) {
                                locationData["guard"] = expressionTranslator.translate(shiftVariablesWrtLowerBound(
                                        getLocationVariable(automaton) ==
                                        this->model.getManager().integer(locationIndex)),
                                                                                       storm::expressions::ToCppTranslationOptions(
                                                                                               variablePrefixes,
                                                                                               variableToName));
                            } else {
                ...
                template<typename ValueType, typename RewardModelType>
                void
                JitFigaroBuilder<ValueType, RewardModelType>::generateLabels(cpptempl::data_map &modelData) {
                    ...
                    for (auto const &variable : model.getGlobalVariables().getTransientVariables()) {
                        if (variable.isBooleanVariable()) {
                            if (this->options.isBuildAllLabelsSet() ||
                                this->options.getLabelNames().find(variable.getName()) !=
                                this->options.getLabelNames().end()) {
                                cpptempl::data_map label;
                                label["name"] = variable.getName();
                                label["predicate"] = expressionTranslator.translate(shiftVariablesWrtLowerBound(
                                        model.getLabelExpression(variable.asBooleanVariable(), parallelAutomata)),
                                                                                    storm::expressions::ToCppTranslationOptions(
                                                                                            variablePrefixes,
                                                                                            variableToName,
                                                                                            storm::expressions::ToCppTranslationMode::CastDouble));
                    ...
                    for (auto const &expressionLabel : this->options.getExpressionLabels()) {
                        ...
                        if (expressionLabelStrings.count(expressionLabelString) == 0) {
                            ...
                            label["predicate"] = expressionTranslator.translate(shiftVariablesWrtLowerBound(expression),
                                                                                storm::expressions::ToCppTranslationOptions(
                                                                                        variablePrefixes,
                                                                                        variableToName,
                                                                                        storm::expressions::ToCppTranslationMode::CastDouble));
                ...
                template<typename ValueType, typename RewardModelType>
                void JitFigaroBuilder<ValueType, RewardModelType>::generateTerminalExpressions(
                        cpptempl::data_map &modelData) {
                    cpptempl::data_list terminalExpressions;

                    for (auto const &terminalEntry : options.getTerminalStates()) {
                        storm::builder::LabelOrExpression const &labelOrExpression = terminalEntry.first;
                        if (labelOrExpression.isLabel()) {
                            auto const &variables = model.getGlobalVariables();
                            STORM_LOG_THROW(variables.hasVariable(labelOrExpression.getLabel()),
                                            storm::exceptions::WrongFormatException,
                                            "Terminal label refers to unknown identifier '"
                                                    << labelOrExpression.getLabel() << "'.");
                            auto const &variable = variables.getVariable(labelOrExpression.getLabel());
                            STORM_LOG_THROW(variable.isBooleanVariable(), storm::exceptions::WrongFormatException,
                            ...
                            STORM_LOG_THROW(variable.isTransient(), storm::exceptions::WrongFormatException,
                                            "Terminal label refers to non-transient variable '" << variable.getName()
                                                                                                << ".");
                            auto labelExpression = model.getLabelExpression(variable.asBooleanVariable(),
                                                                            parallelAutomata);
                            if (!terminalEntry.second) {
                                labelExpression = !labelExpression;
                            }
                            terminalExpressions.push_back(
                                    expressionTranslator.translate(shiftVariablesWrtLowerBound(labelExpression),
                                                                   storm::expressions::ToCppTranslationOptions(
                                                                           variablePrefixes, variableToName)));
                        } else {
                            auto expression = terminalEntry.second ? labelOrExpression.getExpression()
                                                                   : !labelOrExpression.getExpression();
                            terminalExpressions.push_back(
                                    expressionTranslator.translate(shiftVariablesWrtLowerBound(expression),
                                                                   storm::expressions::ToCppTranslationOptions(
                                                                           variablePrefixes, variableToName)));
                        }
                    }
                ...
                template<typename ValueType, typename RewardModelType>
                void JitFigaroBuilder<ValueType, RewardModelType>::generateParameters(
                        cpptempl::data_map &modelData) {
                ...
                template<typename ValueType, typename RewardModelType>
                std::string const &JitFigaroBuilder<ValueType, RewardModelType>::getVariableName(
                        storm::expressions::Variable const &variable) const {
                    return variableToName.at(variable);
                }

                template<typename ValueType, typename RewardModelType>
                std::string const &JitFigaroBuilder<ValueType, RewardModelType>::registerVariable(
                        storm::expressions::Variable const &variable, bool transient) {
                ...
                template<typename ValueType, typename RewardModelType>
                storm::expressions::Variable const &
                JitFigaroBuilder<ValueType, RewardModelType>::getLocationVariable(
                        storm::jani::Automaton const &automaton) const {
                    return automaton.getLocationExpressionVariable();
                }

                template<typename ValueType, typename RewardModelType>
                std::string JitFigaroBuilder<ValueType, RewardModelType>::asString(bool value) const {
                ...
                JitFigaroBuilder<ValueType, RewardModelType>::shiftVariablesWrtLowerBound(
                ...
                template<typename ValueTypePrime>
                std::string
                JitFigaroBuilder<ValueType, RewardModelType>::asString(ValueTypePrime value) const {
                    return std::to_string(value);
                }

                template<typename ValueType, typename RewardModelType>
                std::string JitFigaroBuilder<ValueType, RewardModelType>::createSourceCodeFromSkeleton(
                        cpptempl::data_map &modelData) {
...
#include "storm/exceptions/WrongFormatException.h"
                ...
                template<typename ValueType, typename RewardModelType>
                boost::filesystem::path JitFigaroBuilder<ValueType, RewardModelType>::compileToSharedLibrary(
                        boost::filesystem::path const &sourceHppFile, boost::filesystem::path const &sourceCppFile) {
                    std::string sourceHFilename = boost::filesystem::absolute(sourceHppFile).string();
                    std::string sourceCppFilename = boost::filesystem::absolute(sourceCppFile).string();
                    auto dynamicLibraryPath = sourceHppFile;
                    dynamicLibraryPath += DYLIB_EXTENSION;
                    std::string dynamicLibraryFilename = boost::filesystem::absolute(dynamicLibraryPath).string();
                    std::string includes = "";
                    for (std::string const &dir : {stormIncludeDirectory, sparseppIncludeDirectory,
                                                   boostIncludeDirectory, carlIncludeDirectory, clnIncludeDirectory,
                                                   gmpIncludeDirectory}) {
                        if (dir != "") {
                            includes += " -I " + dir;
                        }
                    }
                    includes = includes + " -I " + STORM_SOURCE_DIR + "/resources/3rdparty/l3pp/ ";
                    std::string command = compiler + " " + sourceCppFile.string() + " " + compilerFlags + includes + " -o " +
                                          dynamicLibraryFilename;
//                    std::cout<<"The compiler command is:"<<command<<std::endl;
                    boost::optional<std::string> error = execute(command);

                    if (error) {
//                        boost::filesystem::remove(sourceHppFile);
//                        boost::filesystem::remove(sourceCppFilename);
                        STORM_LOG_THROW(false, storm::exceptions::InvalidStateException,
                                        "Compiling shared library failed. Error: " << error.get());
                    }
                ...
                RationalFunctionType convertVariableToPolynomial(storm::RationalFunctionVariable const &variable,
                    ...
                    return RationalFunctionType(typename RationalFunctionType::PolyType(
                ...
                RationalFunctionType convertVariableToPolynomial(storm::RationalFunctionVariable const &variable,
                                                                 std::shared_ptr<storm::RawPolynomialCache>) {
                    return RationalFunctionType(variable);
                }

                template<typename ValueType>
                std::vector<storm::RationalFunction>
                getParameters(storm::jani::Model const &, std::shared_ptr<storm::RawPolynomialCache>) {
                    STORM_LOG_THROW(false, storm::exceptions::InvalidStateException,
                                    "This function must not be called for this type.");
                }

                template<>
                std::vector<storm::RationalFunction>
                getParameters<storm::RationalFunction>(storm::jani::Model const &model,
                                                       std::shared_ptr<storm::RawPolynomialCache> cache) {
                    std::vector<storm::RationalFunction> parameters;
                    for (auto const &constant : model.getConstants()) {
                        if (!constant.isDefined() && constant.isRealConstant()) {
                            parameters.push_back(convertVariableToPolynomial<storm::RationalFunction>(
                                    carl::freshRealVariable(constant.getName()), cache));
                        }
                    }
                    return parameters;
                }
                template<typename ValueType, typename RewardModelType>
                void JitFigaroBuilder<ValueType, RewardModelType>::delete_files()
                {std::cout<<"Cleaning files of Jit builder";
#ifdef false
                boost::filesystem::remove(fig2cppIncludeDirectory + "/FigaroModel.cpp");
                boost::filesystem::remove(fig2cppIncludeDirectory + "/FigaroModel.h");
                    boost::filesystem::remove(fig2cppIncludeDirectory + "/FigaroModel.h.dylib");
#endif
                    boost::filesystem::remove(figaroSourceFilePath + "/FigaroModel.cpp");
                    boost::filesystem::remove(figaroSourceFilePath + "/FigaroModel.h");
                    boost::filesystem::remove(figaroSourceFilePath + "/FigaroModel.h.dylib");

                }
                template<typename ValueType, typename RewardModelType>
                std::shared_ptr<storm::figaro::FigaroProgram> JitFigaroBuilder<ValueType, RewardModelType>::createFigaroModel(
                        boost::filesystem::path const &dynamicLibraryPath) {
//                    jitBuilderCreateFunction = boost::dll::import_alias<typename JitFigaroBuilder<ValueType, RewardModelType>::CreateFunctionType>(
//                            dynamicLibraryPath, "create_builder");
//                    builder = std::shared_ptr<storm::figaro::FigaroProgram> (jitBuilderCreateFunction());
                    creator =  boost::dll::import_alias<CreateFunctionType>(             // type of imported symbol must be explicitly specified
                            dynamicLibraryPath,                                            // path to library
                            "create_builder",                                                // symbol to import
                            boost::dll::load_mode::append_decorations                              // do append extensions and prefixes
                    );
                    std::shared_ptr<storm::figaro::FigaroProgram> builder = creator();
                    return builder;
//                    if (std::is_same<storm::RationalFunction, ValueType>::value) {
...
//
//                        ImportInitializeParametersFunctionType initializeParametersFunction = boost::dll::import_alias<InitializeParametersFunctionType>(
...
//                    }
                ...
                template
                class JitFigaroBuilder<double, storm::models::sparse::StandardRewardModel<double>>;

                template
                class JitFigaroBuilder<storm::RationalNumber, storm::models::sparse::StandardRewardModel<storm::RationalNumber>>;

                template
                class JitFigaroBuilder<storm::RationalFunction, storm::models::sparse::StandardRewardModel<storm::RationalFunction>>;
