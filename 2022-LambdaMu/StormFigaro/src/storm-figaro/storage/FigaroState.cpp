...
//            }
            ...
            template<typename ValueType>
            FigaroState<ValueType>::FigaroState(storm::storage::BitVector const &status,storm::storage::BitVector const &symmetricStatus, size_t id):
            mStatus(status), mId(id),mPseudoState(false),mSymmetricStatus(symmetricStatus){ //shahid: we are not coonsidering any pseudo state
