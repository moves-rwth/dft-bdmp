#include "FigaroSimulationFileParser.h"
...
namespace storm {
    namespace figaro {
        namespace parser {
            ...
            FigaroSimulationFileParser::FigaroSimulationFileParser(std::string analysisFileName) : analysisFileName(analysisFileName) {

            }
            std::deque<std::deque<std::set<std::string>>> FigaroSimulationFileParser::getSimulationTraces() {
                return simulationTraces;
            }

            void FigaroSimulationFileParser::parse() {
                try {
...
//                    std::cout << node << "   " << name << std::endl;
                    if (name == "PROBABILITES_SEQUENCES") {
...
//                            std::cout << child_node << "   " << child_name << std::endl;
                            if (child_name == "SEQUENCE") {
                                std::deque<std::set<std::string>> temp_trace;
                                for (uint_fast64_t k = 0; k < child_node->getChildNodes()->getLength(); ++k) {
                                    auto seq_node = child_node->getChildNodes()->item(k);
                                    auto seq_name = storm::adapters::getName(seq_node);
//                                    std::cout << seq_node << "   " << seq_name << std::endl;
                                    if (seq_name == "BRANCHE") {
                                        std::set<std::string> temp_transition;
                                        for (uint_fast64_t m = 0; m < seq_node->getChildNodes()->getLength(); ++m) {
                                            auto branch_node = seq_node->getChildNodes()->item(m);
                                            auto branch_name = storm::adapters::getName(branch_node);
//                                            std::cout << branch_node << "   " << branch_name << std::endl;
                                            if (branch_name == "TRANSITION"){

                                                std::string objectName;
                                                for (uint_fast64_t n = 0; n < branch_node->getAttributes()->getLength(); ++n) {
                                                    auto attr = branch_node->getAttributes()->item(n);
                                                    auto attr_name = storm::adapters::getName(attr);
//                                                    std::cout << attr << "   " << attr_name << std::endl;
                                                    if (attr_name == "OBJET") {
                                                        objectName = storm::adapters::XMLtoString(attr->getNodeValue());
                                                    }
                                                    if (attr_name == "TRANS"){
//                                                        std::cout<<objectName + "_"+storm::adapters::XMLtoString(attr->getNodeValue());
                                                        temp_transition.insert(objectName + "_"+storm::adapters::XMLtoString(attr->getNodeValue()));
                                                    }
                                            }
                                        ...
                                        temp_trace.push_back(temp_transition);
                                    }
                                }
                                if(temp_trace.size()>0) {simulationTraces.push_back(temp_trace);}
                            }
...
//                for (int i = 0; i < simulationTraces.size(); i++)
//                {
//                    std::cout<<std::endl;
//                    for (int j = 0; j < simulationTraces[i].size(); j++)
//                    {
//                        std::cout << simulationTraces[i][j]<< "   :    ";
//                    }
//                    std::cout<<std::endl;
//                }
            }
            ...
            void FigaroSimulationFileParser::traverseProjectElement(xercesc::DOMNode const *const node) {
            // traverse attributes
        ...
        void FigaroSimulationFileParser::traverseGspnElement(xercesc::DOMNode const *const node) {
            // traverse attributes
            for (uint_fast64_t i = 0; i < node->getAttributes()->getLength(); ++i) {
                auto attr = node->getAttributes()->item(i);
                auto name = storm::adapters::getName(attr);
        ...
        void FigaroSimulationFileParser::traverseNodesElement(xercesc::DOMNode const *const node) {
            // traverse attributes
            for (uint_fast64_t i = 0; i < node->getAttributes()->getLength(); ++i) {
                auto attr = node->getAttributes()->item(i);
                auto name = storm::adapters::getName(attr);
        ...
        void FigaroSimulationFileParser::traverseConstantOrTemplateElement(xercesc::DOMNode const *const node) {
        ...
        void FigaroSimulationFileParser::traverseEdgesElement(xercesc::DOMNode const *const node) {
            // traverse attributes
            for (uint_fast64_t i = 0; i < node->getAttributes()->getLength(); ++i) {
                auto attr = node->getAttributes()->item(i);
                auto name = storm::adapters::getName(attr);
        ...
        void FigaroSimulationFileParser::traversePlaceElement(xercesc::DOMNode const *const node) {
            // traverse attributes
        ...
        void FigaroSimulationFileParser::traverseTransitionElement(xercesc::DOMNode const *const node) {
        ...
        void FigaroSimulationFileParser::traverseArcElement(xercesc::DOMNode const *const node) {
...
#endif
//        int64_t FigaroSimulationFileParser::parseInt(std::string str) {
//            expressionParser->setAcceptDoubleLiterals(false);
...
//        }
//
//        double FigaroSimulationFileParser::parseReal(std::string str) {
//            expressionParser->setAcceptDoubleLiterals(true);
//            auto expr = expressionParser->parseFromString(str).substitute(constantsSubstitution);
//            STORM_LOG_ASSERT(!expr.containsVariables(),
//                             "Can not evaluate expression " << str << " as it contains undefined variables.");
