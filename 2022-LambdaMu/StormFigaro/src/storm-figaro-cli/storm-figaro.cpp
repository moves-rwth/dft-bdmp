...
void processOptions() {
    ...
    auto ioSettings =  storm::settings::getModule<storm::settings::modules::IOSettings>();
        ...
        boost::function<CreateFunctionType> creator_dummy;

    storm::figaro::builder::jit::JitFigaroBuilder<double> jitbuilder(figarooptions.getFigaroFilename(), figarooptions.getxmlFilename());

//    std::shared_ptr<storm::figaro::FigaroProgram> figaromodel = storm::figaro::api::loadFigaroProgram(figarooptions.getFigaroFilename(), figarooptions.getxmlFilename());
    std::shared_ptr<storm::figaro::FigaroProgram> figaromodel = storm::figaro::api::loadFigaroProgram(jitbuilder);
     jitbuilder.getcreator(creator_dummy);
    ...
    //TODO Similiar to storm-DFT serveral switches are allowed
    ...
    if (figarooptions.isYamsMcrFileSet() and false)
    {
        STORM_LOG_DEBUG("Get Simulation traces from the Mcr file " << figarooptions.getYamsMcrFilename());
        std::deque<std::deque<std::set<std::string>>> traces;
        traces = storm::figaro::api::parseYamsSimulationTraces(figarooptions.getYamsMcrFilename());
...
//    std::shared_ptr<storm::models::sparse::Model<ValueType>> model; //merge this
    ...
    storm::figaro::api::analyzeFigaro<ValueType>(figaromodel, props, approximationError,
                                                       figarooptions.getApproximationHeuristic(), true, figarooptions.isApproximationErrorSet(), figarooptions.isTopNonRepairableAnalysisOptionSet());
