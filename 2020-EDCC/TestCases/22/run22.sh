...
res1=$(date +%s.%N)
python3 ../../PythonScript/src/main.py -i '22.fi' -o 'DummyForPNPRO.modest'
...
/usr/local/GreatSPN/bin/DSPN-Tool -load "../BENCHMARK/BENCHMARK-Measures.solution//final" -epsilon 1.0E-7 -on-the-fly -i -gmres -dta-path "../BENCHMARK/BENCHMARK-Measures.solution" -cslta0-X MEASURE0 "PROB_TA>0 DTA_unr (t=5| | Phi1=#FAIL>0 )"
/usr/local/GreatSPN/bin/DSPN-Tool -load "../BENCHMARK/BENCHMARK-Measures.solution//final" -epsilon 1.0E-7 -on-the-fly -i -gmres -dta-path "../BENCHMARK/BENCHMARK-Measures.solution" -cslta0-X MEASURE0 "PROB_TA>0 DTA_unavil (t=5| | Phi1=#FAIL>0 )"
res3=$(date +%s.%N)
...
#storm --jani ../BENCHMARK/temp.jani --prop "Pmin=? [F<=5 FAIL>0] "  --precision 1e-6
