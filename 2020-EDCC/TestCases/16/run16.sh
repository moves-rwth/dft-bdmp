...
res1=$(date +%s.%N)
python3 ../../PythonScript/src/main.py -i '16.fi' -o 'DummyForPNPRO.modest'
...
#storm --jani ../BENCHMARK/temp.jani --prop "Pmin=? [F<=10 FAIL>0];Pmin=? [F<=10 FAIL>0];Pmax=? [F<=10 FAIL>0]"  --buildfull --buildstateval --precision 1e-6
